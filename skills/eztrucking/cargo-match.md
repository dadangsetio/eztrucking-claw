...
## Tools
`send` (ask the vendor about fit), `memory`/`skill` write. **NO money/amount tool.**
...
2. If fit unknown/stale, ask the vendor about capacity, goods type, special handling. Example WhatsApp:
   - "Pak, ada truk untuk muatan sekitar 8 ton ke Makassar?"
   - "Barangnya elektronik, butuh yang tertutup. Bisa handle?"
