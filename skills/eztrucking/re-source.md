...
`web_search`/`web_fetch`/managed `browser` (find alternative). `message(action="send", to="<phone>", message="<text>")` — re-quote the alternative. `memory` write. **NO money/amount tool.**
...
3. Re-quote the alternative. Example WhatsApp:
   - `message(action="send", to="<vendor_phone>", message="Pak, masih ada armada untuk Surabaya–Makassar minggu ini?")`
   - `message(action="send", to="<vendor_phone>", message="Muatan 8 ton, butuh cepat. Bisa bantu?")`
